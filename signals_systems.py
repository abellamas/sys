...
def bandpass_filter(x, f_s, b=1, G=2, N_order=4, worN=5120):
    ...
    N_order : INTEGER, optional
        The order of the filter. Parameter of signal.butter(). The default is 4.
...
def filter_bank(audio_input, f_s, p_ref=0.00002, b=1, G=2, N_order=4):
...
def frequency_labels(octave:str):
    ...
    if octave == 'octaves':
        norm_freqs = [31.25, 62.5, 125, 250, 500, 1000, 2000, 4000, 8000, 16000]
        label_freqs = ['31.25', '62.5', '125', '250', '500', '1k', '2k', '4k', '8k', '16k']
    elif octave == 'thirds':
        ...
        label_freqs = ['25', '31.5', '40', '50', '63', '80', '100', '125', '160', '200', '250', '315', '400', '500', '630', '800', '1k', '1.25k', '1.6k', '2k', '2.5k', '3.15k', '4k', '5k', '6.3k', '8k', '10k', '12.5k', '16k', '20k']
    elif octave == 'decades':
        norm_freqs = [1, 10, 100, 1000, 10000, 10000]
        label_freqs = ['1', '10', '100', '1000', '10000', '10000']
        
    return norm_freqs, label_freqs


def grapher(signals:list, plot:str = 'normal', dimension:tuple = (1,1), oct_axis:str = 'octaves', **options):
    """
    ACTUALIZAR DOC!!!
    ...
    oct_axis : str
        To plots of bars or semilogx can be set the x axis by 'octaves', 'thirds' or 'decades'. By default is 'octaves'.
    ...
    **options
    ...
    if dimension == (1,1):
        for sg in signals:
            ...
            if plot=='semilogx':
                axs.semilogx(x_values, y_values, label= options['legends'][i] if 'legends' in options else None)
                freqs_ticks, freqs_label = frequency_labels(oct_axis)
                axs.minorticks_off()
                axs.set_xticks(freqs_ticks)
                axs.set_xticklabels(freqs_label, rotation=45 if oct_axis=='thirds' else 0)
                ...
                ValueError('The type of plot specified does not exist')
            i = i + 1
    ...
    #             index_freqs = np.array(list(range(-5*oct_axis-int(oct_axis/3), 5*oct_axis-int(oct_axis/3), 1)))
    #             axis_freqs = f_m(x=index_freqs, b=oct_axis)
    #             ax.bar(range(len(x_values)), y_values, color=options['color'] if 'color' in options else 'g')
